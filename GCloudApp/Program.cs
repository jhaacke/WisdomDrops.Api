...
//						 services.AddSingleton<ILoggerProvider>( sp => GoogleLoggerProvider.Create( GcpProjectId ) );
						 services.AddScoped<IDomainContext>( provider => new DomainContext( Configuration.GetValue<string>( "Google:ProjectId" ) ) );
						 services.AddScoped<IExcelAdapter, ExcelAdapter>();
						 services.AddScoped<ICoreServices, CoreServices>();
						 services.AddScoped<ISmsAdapter>( provider => new TwilioSmsAdapter( 
							 Configuration.GetValue<string>( "Twilio:AccountId" ),
							 Configuration.GetValue<string>( "Twilio:AuthToken" ),
							 Configuration.GetValue<string>( "Twilio:PhoneNumber" ) ) );
...
//					 }
		...
		private static void _SetCredentialFile()
		{
			const string FILE_NAME = "Another Test Project-fa8bd0ae3c3c.json";
			const string ENV_VAR = "GOOGLE_APPLICATION_CREDENTIALS";
